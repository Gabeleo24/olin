...
4. **Env Variables**
   ...
   - API runtime: access to `data/colleges.db` plus:
     - `SUPABASE_JWT_SECRET`
     - `SUPABASE_URL`
     - `SUPABASE_SERVICE_ROLE_KEY`
     - `SUPABASE_STORAGE_BUCKET` (defaults to `profiles`)
     - Optional `SUPABASE_STORAGE_PUBLIC_URL` if you front the bucket with a CDN.
5. **Monitoring**
...
## 7. Database Cache Distribution

The ETL generates a large `data/colleges.db` (>100 MB). Rather than storing it in git, we publish the fresh cache as a GitHub Release asset.

### Automatic publishing
- Weekly refresh workflow (`.github/workflows/data-refresh.yml`) now:
  1. Runs all enrichment scripts.
  2. Uploads the outputs as a workflow artifact (_colleges-cache_).
  3. Creates/updates a prerelease tagged `latest-cache`, attaching:
     - `colleges.db`
     - `location_states.*`
     - `location_cities.*`
     - `teleport_fallback.json`
     - `schools.html`
- Use the default GitHub token—no extra secrets required.

### Downloading the full DB locally
1. Ensure `python -m pip install -r requirements.txt` has run (only needs stdlib for urllib).
2. Execute:
   ```bash
   python scripts/download_cache.py --tag latest-cache --dest data
   ```
   - Set `GITHUB_TOKEN` (classic PAT or fine-grained) if the repo is private to avoid rate limits.
3. After the files land in `data/`, launch the stack:
   ```bash
   docker compose up
   ```

### Custom download options
- `scripts/download_cache.py --files colleges.db location_states.json` lets you pull only what you need.
- To pin a specific historical run, use the release tag generated by the workflow (e.g., `2024-11-21-refresh` if you extend the workflow to tag per date).
